...
const AboutContentOne = () => {
  ...
  const PackageCard = ({
    ...
    >
      ...
      />
      {/* Default Gradient Overlay - Bottom to Top */}
      ...
      <div className="absolute bottom-0 left-0 w-full p-5 text-white z-10">
        <h2
          className={`primaryFont font-bold line-clamp-1 tracking-wider ${
        ...
        <p
          className={`font-semibold ${
            isMobile ? "text-sm mt-2" : "text-sm mt-3"
          }`}
        >
          {card.category}
        </p>
      ...
      >
        <h3
          className={`uppercase text-white tracking-wide font-semibold text-center ${
      ...
      <div className="max-w-[1350px] mx-auto w-full px-5 md:px-8">
        ...
        <Button
          ...
          className={`border-2 text-black ${
            isMobile ? "w-full mt-6 mb-20" : "w-fit mt-5 md:mt-8"
          }`}
        />
      </div>
    </div>
